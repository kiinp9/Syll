<script lang="ts">
    ...
    const handleGetDropDownData: SubmitFunction = () => {
        return async ({ result }) => {
            if (result.type === 'success' && result.data) {
                const response = result.data as any;
                if(response.status === 1 && response.data) {
                    dropDownDataList = response.data;
                }
            }
        }
    }
...
<form method="POST" action="?/getListDropDownFormLoai" use:enhance={handleGetDropDownData} hidden>
    <button bind:this={btnSubmitDropDown} type="submit"></button>
</form>
        ...
        <CardContent class="p-6 pt-1 space-y-4">
            ...
            <div class="relative">
                    ...
                    <Input
                        ...
                        placeholder="Tìm kiếm theo tên tổ chức, loại tổ chức hoặc mã số tổ chức ..."
                        class="pl-10"
                    />
                </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                <thead class="border-b bg-gray-50">
                                    <tr>
                                        <th class="text-center p-4 font-semibold  text-gray-700">Họ và tên</th>
                                        <th class="text-center p-4 font-semibold text-gray-700">Email</th>
                                        <th class="text-center p-4 font-semibold text-gray-700">Đơn vị</th>
                                        <th class="text-center p-4 font-semibold text-gray-700">Role</th>
                                    ...
                                    {#if data.nhanVien.data.length > 0}
                                        {#each data.nhanVien.data as member}
                                            <tr class="border-b hover:bg-gray-50 transition-colors">
                                                <td class="p-4 text-center font-medium">{member.hoVaTen}</td>
                                                <td class="p-4 text-center text-gray-600">{member.email}</td>
                                                <td class="p-4 text-center" >
                                                    <span class="inline-flex  items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                                                        {member.toChuc?.tenToChuc || 'N/A'}
                                                    </span>
                                                </td>
