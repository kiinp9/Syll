...
<script lang="ts">
    ...
    const onUpdateForm = getContext<(data: IViewForm) => void>('onUpdateForm');
	const onDeleteForm = getContext<(data: IViewForm) => void>('onDeleteForm');
   const onViewFormDetail = getContext<(idFormLoai: number) => void>('onViewFormDetail');
	...
	function handleUpdate() {
		...
		isOpenPopover = false;
        if (onUpdateForm) onUpdateForm(rowData);
	}
	...
	function handleDelete() {
		...
		isOpenPopover = false;
        if (onDeleteForm) onDeleteForm(rowData);
	}
...
<Popover.Root bind:open={isOpenPopover}>
	...
	<Popover.Content class="w-40 p-0" align="end">
		<div class="flex flex-col">
			...
			<button 
				type="button" 
				class="px-4 py-2 text-left text-sm hover:bg-gray-100 cursor-pointer text-red-600 flex items-center gap-2"
